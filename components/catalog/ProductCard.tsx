...
import { useChatStore, useCatalogStore } from "@/lib/store";
import { MessageCircle, Eye, BarChart3 } from "lucide-react";
...
export function ProductCard({ product }: { product: Product }) {
  ...
  const { compareIds, toggleCompare, setQuickViewProduct } = useCatalogStore();
  ...
  return (
    <article className="group bg-white rounded-2xl overflow-hidden border border-zinc-200/60 transition-all duration-300 hover:shadow-[0_8px_30px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-0.5 flex flex-col">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <Link href={`/product/${product.id}`}>
          <h3 className="font-semibold text-zinc-900 text-[1.05rem] leading-snug mb-1.5 line-clamp-1 tracking-tight hover:text-emerald-600 transition-colors">
            {product.name}
          </h3>
        </Link>
        ...
        {isAvailable && (
          <div className="mt-3 mb-1">
            ...
            <div className="w-full bg-zinc-100 rounded-full h-1.5 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ${stockPercent > 20 ? "bg-emerald-500" : "bg-red-500"}`}
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-zinc-100">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium text-zinc-500 hover:text-emerald-600 hover:bg-emerald-50 px-2.5 py-1.5 rounded-full border border-zinc-200 hover:border-emerald-200 transition-all active:scale-95"
            ...
            <button
              ...
              className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-full border transition-all active:scale-95 ${
                isComparing
                  ? "bg-emerald-500 text-white border-emerald-500"
                  : "text-zinc-500 hover:text-emerald-600 hover:bg-emerald-50 border-zinc-200 hover:border-emerald-200"
              }`}
              aria-label="Compare"
            >
